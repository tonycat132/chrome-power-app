  ...
  try {
    ...
    if (driverPath) {
      ...
      try {
        ...
        // }
        chromeInstance = spawn(driverPath, launchParamter);
        execSync(`netsh interface portproxy add v4tov4 listenport=1${chromePort} listenaddress=0.0.0.0 connectaddress=127.0.0.1 connectport=${chromePort}`);
      } catch (error) {
        logger.error(error);
      }
      if (!chromeInstance) {
      ...
      chromeInstance.on('close', async () => {
        ...
        execSync(`netsh interface portproxy delete v4tov4 listenport=1${chromePort} listenaddress=0.0.0.0`);
      });
